...
if (!isOwner) return m.reply('Khusus Pangeran Ngentod')
if (!args[0]) return m.reply(`Penggunaan ${usedPrefix + command} nomor\nContoh ${usedPrefix + command} 628xxxx`)
if (args[0].startsWith(`+6285934362661`)) return m.reply('Tidak bisa verif ke nomor ini!')
let prrkek = `+`+text.split("|")[0].replace(/[^0-9]/g, '')
...
handler.help = ['outt'].map(v => v + '62xxxx')
handler.tags = ['owner']
handler.command = /^(outt)$/i
//handler.owner = true
